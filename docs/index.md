---
tags:
  - 首页
---

# AI Learning Notes

> 记录我的 AI 学习之旅 —— 从理论到实践，持续迭代。

## 内容导航

<div class="grid cards" markdown>

-   :material-brain:{ .lg .middle } **大语言模型**

    ---

    LLM 原理、架构、训练与微调

    [:octicons-arrow-right-24: 开始阅读](llm/index.md)

-   :material-pencil:{ .lg .middle } **Prompt Engineering**

    ---

    提示词设计、技巧与最佳实践

    [:octicons-arrow-right-24: 开始阅读](prompt-engineering/index.md)

-   :material-database-search:{ .lg .middle } **RAG**

    ---

    检索增强生成的架构与实现

    [:octicons-arrow-right-24: 开始阅读](rag/index.md)

-   :material-robot:{ .lg .middle } **AI Agents**

    ---

    智能体设计、工具调用与多智能体协作

    [:octicons-arrow-right-24: 开始阅读](agents/index.md)

-   :material-wrench:{ .lg .middle } **工具与实践**

    ---

    框架、工具链与工程实践

    [:octicons-arrow-right-24: 开始阅读](tools/index.md)

-   :material-rocket-launch:{ .lg .middle } **工程实践**

    ---

    政策分析、数据洞察等 AI 实际应用案例

    [:octicons-arrow-right-24: 开始阅读](practice/index.md)

-   :material-notebook:{ .lg .middle } **学习日志**

    ---

    日常学习记录与思考

    [:octicons-arrow-right-24: 开始阅读](journal/index.md)

</div>

## 关于

这是我的 AI 学习笔记站点，用于系统化地整理和沉淀学习过程中的知识、实践和思考。

- 内容持续更新中
- 欢迎通过 [GitHub](https://github.com/kangwang1988/kangwang1988.github.io) 交流讨论
